...
describe MoviesController do
	...
	let(:valid_attributes){{:title => "Teste", :rating => "PG", :description => "A" , :release_date =>"B", :director => "The" }}
	...
	describe 'Find movies by same director' do
	  ...
	  it 'When I go to the show page for "Alien"' do 
		movie = Movie.create! valid_attributes
      	get :similar, {:movie_id => movie.to_param}	
      	expect(assigns(:movie)).to eq(movie)
  	  end
  	  ...
  	  it 'I should be on the Similar Movies page for the Movie' do
      	...
      	Movie.should_receive(:similar_directors).with(double.director).and_return(similarDouble)
      	get :similar, {:movie_id => '13'}
      end
